...
@pytest.mark.asyncio
async def test_start_battle(mock_pokemon_data_manager):
    mock_background_tasks = Mock(BackgroundTasks)
    battle_simulator = BattleSimulator()
    pokemon_controller = PokemonController(mock_pokemon_data_manager, battle_simulator)
...
@pytest.mark.asyncio
async def test_get_battle_result(mock_pokemon_data_manager):
    mock_background_tasks = Mock(BackgroundTasks)
